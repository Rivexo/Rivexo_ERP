...
// queda por cobrar y cuánto de eso está vencido. Se deriva en vivo de
// deal_payment_installments, igual que getAccountsReceivable.
export async function listProjectsFinancialSummary(): Promise<ProjectFinancialSummaryRow[]> {
  ...
  const { data, error } = await supabase
    ...
    .select(
      ...
      financials:project_financials(budget_sold),
  ...
  type Row = {
    ...
    financials: { budget_sold: number } | null;
  ...
  return (data as unknown as Row[]).map((row) => {
    ...
    return {
      ...
      budget_sold: row.financials?.budget_sold ?? 0,
      collected,
      receivable,
      overdue,
    };
